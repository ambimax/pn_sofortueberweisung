 ...
 *
 * $Date: 2012-06-06 10:13:59 +0200 (Mi, 06. Jun 2012) $
 * @version SofortLib 1.5.0  $Id: sofortLib.php 4354 2012-06-06 08:13:59Z dehn $
...
class SofortLib {
	...
	protected $_products = array('all', 'sr', 'su', 'sv', 'sa', 'ls', 'sl', 'sf');
	...
	protected function _handleErrors() {
		...
		//handle warnings
		if (isset($this->_response['warnings']['warning'])) {
			if (!isset($this->_response['warnings']['warning'][0])) {
				$tmp = $this->_response['warnings']['warning'];
				unset($this->_response['warnings']['warning']);
				$this->_response['warnings']['warning'][0] = $tmp;
			}
			
			foreach ($this->_response['warnings']['warning'] as $warning) {
				$this->warnings['global'][] = $this->_getErrorBlock($warning);
			}
		}
		
		foreach ($this->_products as $product) {
			if (isset($this->_response['warnings'][$product])) {
				if (!isset($this->_response['warnings'][$product]['warnings']['warning'][0])) {
					$tmp = $this->_response['warnings'][$product]['warnings']['warning'];
					unset($this->_response['warnings'][$product]['warnings']['warning']);
					$this->_response['warnings'][$product]['warnings']['warning'][0] = $tmp;
				}
			
				foreach ($this->_response['warnings'][$product]['warnings']['warning'] as $error) {
					$this->warnings[$product][] = $this->_getErrorBlock($error);
				}
			}
		}
	}
	...
	private function _getErrorBlock($error) {
