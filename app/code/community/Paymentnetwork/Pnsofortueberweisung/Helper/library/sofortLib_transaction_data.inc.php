 ...
 * @version SofortLib 1.3.0  $Id: sofortLib_transaction_data.inc.php 4972 2012-08-01 09:05:46Z dehn $
...
class SofortLib_TransactionData extends SofortLib_Abstract {
	...
	public function getInvoiceAddress($i = 0) {
		...
		$invoiceAddress = array(
			...
			'salutation' => $this->_response[$i]['sr']['invoice_address']['salutation']['@data'],
		);
	...
	public function getShippingAddress($i = 0) {
		...
		$shippingAddress = array(
			...
			'salutation' => $this->_response[$i]['sr']['shipping_address']['salutation']['@data'],
		);
	...
	public function getSofortdauerauftragPaymentReceived($i = 0, $j = 0) {
		if ($i < 0 || $i >= $this->_count || $this->_response[$i]['payment_method']['@data'] != 'sa' || $j < 0 || ($j > count($this->_response[$i]['sa']['payments']['payment']) -1)) {
			return false;
		}
